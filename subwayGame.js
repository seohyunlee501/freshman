    ...
    if (this.everyone[this.idx].die) {
      ...
      if (!this.turnStarted) {
        if (this.falseProbability[0] < 0.5) {
          let lineIdx = Math.floor(
            random(0, this.stationList[this.currentLine].length)
          );
  ...
  gameend() {
    if (!this.endStarted) {
      ...
      if (this.loseIssue == "입력") {
        ...
        } else if (millis() - this.endTime < 4000) {
          this.gameFinishedText("집중은 생명!");
        } else if (millis() - this.endTime < 5200) {
          this.gameFinishedText("생명! 생명!!");
        } else if (millis() - this.endTime < 6400) {
